      ...
      list = feedlist.where((x) {
        ...
        /* if (x.user.userId == userModel.userId ||
            (userModel?.followingList != null &&
                userModel.followingList.contains(x.user.userId)) || X.) {
          return true;
        } else {
          return false;
        }*/
  ...
  Future<bool> databaseInit() {
    try {
      if (_feedQuery == null) {
        ...
        _feedQuery.onChildChanged.listen(_onTweetAdded);
  ...
  Future<void> deleteFile(String url, String baseUrl) async {
    try {
      String filePath = url.replaceAll(
          new RegExp(
              r'https://firebasestorage.googleapis.com/v0/b/ecuadoramalavida-f8ae1.appspot.com/o/'),
          '');
